...
import { Search, Mail, Bell, User, Menu, ChevronRight, ChevronLeft, Star, Plus, LogOut, Settings, Package, X } from "lucide-react"
            ...
            <div className="absolute right-0 top-0 h-full w-64 bg-white shadow-lg">
              <div className="p-4 flex flex-col gap-4">
                ...
                <nav className="flex flex-col gap-4">
                  <Link 
                    href="/browse" 
                    className="p-2 hover:bg-gray-100 rounded-lg font-medium"
                    onClick={() => setIsProfileOpen(false)}
                  >
                    Browse
                  </Link>
                ...
                {user && (
                  <div className="mt-4 pt-4 border-t">
                    <button
                      ...
                      className="w-full p-2 text-red-600 hover:bg-red-50 rounded-lg font-medium flex items-center gap-2"
                    >
          ...
          <div className="container mx-auto px-4 py-8">
            ...
            <div className="mb-12">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl sm:text-2xl font-bold">Featured Shoes</h2>
                <div className="flex gap-2">
                  <button
                    onClick={() => handlePrevPage('shoes')}
                    disabled={currentShoesPage === 0}
                  ...
                  <button
                    ...
                    disabled={currentShoesPage === shoesPages.length - 1}
                    className="p-2 rounded-full hover:bg-gray-100 disabled:opacity-50 disabled:hover:bg-transparent"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {shoesPages[currentShoesPage]?.map((product: any) => (
            ...
            <div className="mb-12">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl sm:text-2xl font-bold">Featured Clothing</h2>
                <div className="flex gap-2">
                  <button
                    onClick={() => handlePrevPage('clothing')}
                    disabled={currentClothingPage === 0}
                    className="p-2 rounded-full hover:bg-gray-100 disabled:opacity-50 disabled:hover:bg-transparent"
                  >
                    <ChevronLeft className="w-5 h-5" />
                  </button>
                  <button
                    ...
                    disabled={currentClothingPage === clothingPages.length - 1}
                    className="p-2 rounded-full hover:bg-gray-100 disabled:opacity-50 disabled:hover:bg-transparent"
                  >
                    <ChevronRight className="w-5 h-5" />
                  </button>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {clothingPages[currentClothingPage]?.map((product: any) => (
                  <ProductCard key={product.id} {...product} />
                ))}
              </div>
            </div>
            ...
            <div className="mb-12">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl sm:text-2xl font-bold">Featured Accessories</h2>
                <div className="flex gap-2">
                  <button
                    onClick={() => handlePrevPage('accessories')}
                    disabled={currentAccessoriesPage === 0}
                    className="p-2 rounded-full hover:bg-gray-100 disabled:opacity-50 disabled:hover:bg-transparent"
                  >
                    <ChevronLeft className="w-5 h-5" />
                  </button>
                  <button
                    ...
                    disabled={currentAccessoriesPage === accessoriesPages.length - 1}
                    className="p-2 rounded-full hover:bg-gray-100 disabled:opacity-50 disabled:hover:bg-transparent"
                  >
                    <ChevronRight className="w-5 h-5" />
                  </button>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {accessoriesPages[currentAccessoriesPage]?.map((product: any) => (
                  <ProductCard key={product.id} {...product} />
                ))}
              </div>
            </div>
...
const ProductCard: React.FC<ProductCardProps> = ({ id, images, name, price, rating, className }) => {
  return (
    <Link href={`/product/${id}`} className={cn("group relative bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200", className)}>
      ...
      <div className="p-4">
        <h3 className="text-sm sm:text-base font-medium text-gray-900 line-clamp-2">{name}</h3>
        <div className="mt-2 flex items-center justify-between">
          <p className="text-sm sm:text-lg font-semibold text-gray-900">৳{price.toFixed(2)}</p>
